function calculate_TimePerHopVsDistSuper(Filename,timePer,dis)
% This function is designed to read in .xyz format file where the 4th
% column represents the hop frequencies and the times spent on each site 
% from there it calculate the time per hop spent on each site it then
...
% makes a 3d plot
    ...
    TimePerVisit = ones(Row,Col,Shelf);
    ...
    for i=1:Row
        for j=1:Col
            ...
            var3 = Times( (1+((i-1)*Row*Col)+(j-1)*Col):((i-1)*Row*Col)+j*Col);
            var2 = Dist( (1+((i-1)*Row*Col)+(j-1)*Col):((i-1)*Row*Col)+j*Col);
            TimePerVisit(i,j,:) = reshape(var3./var,[1,1,Shelf]);
    ...
    p = patch(isosurface(X,Y,Z,TimePerVisit,timePer));
