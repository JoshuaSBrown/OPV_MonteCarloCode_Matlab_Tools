function ZipfLaw(FileName,color)
...
h = figure(1);
set(h,'Position',[10 10 600 400]);
set(gcf,'Color','w');
% subplot(3,1,1);
% hold on
% bar([1:finish],freq(1:finish),color,'edgecolor', 'none', 'BarWidth', 1);
% ylabel('Frequency');
% xlabel('Site');
% subplot(3,1,2);
% hold on;
...
% subplot(3,1,3);
hold on;
bar([1:finish],freq(1:finish),'FaceColor',color,'edgecolor', 'none', 'BarWidth', 1);
xlim([1 finish])
set(gca, 'Yscale',  'log')
set(gca, 'Xscale',  'log')
ylabel('Frequency');
