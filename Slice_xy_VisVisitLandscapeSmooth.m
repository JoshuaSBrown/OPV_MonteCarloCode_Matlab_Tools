function Slice_xy_VisVisitLandscapeSmooth( FileName, z )
...
for i=1:Col
    X(:,i) = X(:,i).*rangeX';
    Visit(:,i) = smooth((D((1+((i-1)*Row)):(i*Row),4))');
end
for i=1:Row
    Y(i,:) = Y(i,:).*rangeY;
    Visit(i,:) = smooth(Visit(i,:));
end
figure(1);

h = surf(X(2:Row-1,2:Col-1),Y(2:Row-1,2:Col-1),log(Visit(2:Row-1,2:Col-1)));
